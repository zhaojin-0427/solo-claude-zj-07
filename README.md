...
- `lacing.first_spoke`：避开阀孔的首根定位——取阀孔角序下一圈孔，
  相位自动二选一使阀孔净空最大；
- `lacing.flange_shift`：自动穿法为每侧法兰搜索到的对齐（孔位双射的整体旋转）；
...
自动穿法按**实际孔位**搜索方向相位与每侧法兰对齐，使交叉方向、交叉数
（按实际夹角折算）与阀孔净空都满足设置，取净空最大者；无解时返回
`LACING_INFEASIBLE` 及冲突孔（孔号、侧别、原因）。
...
孔表覆盖/编号重复、圆周角重复、侧别数量、法兰相位与逐孔角度互斥及覆盖等
输入校验由 Pydantic 完成，返回 422 及具体字段位置。
